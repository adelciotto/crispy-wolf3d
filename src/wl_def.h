...
// TODO(Anthony): Change this to button letters for console versions
...
typedef enum { di_north, di_east, di_south, di_west } controldir_t;
...
typedef enum { ac_badobject = -1, ac_no, ac_yes, ac_allways } activetype;
...
typedef enum { dr_open, dr_closed, dr_opening, dr_closing } doortype;
...
typedef struct {
	...
	short episode, secretcount, treasurecount, killcount, secrettotal,
...
#define PlaySoundLocTile(s, tx, ty)                                            \
	PlaySoundLocGlobal(s, (((fixed)(tx) << TILESHIFT) + (TILEGLOBAL / 2)),     \
					   (((fixed)ty << TILESHIFT) + (TILEGLOBAL / 2)))
...
#define DEMOCHOOSE_ORIG_SDL(orig, sdl)                                         \
	((demorecord || demoplayback) ? (orig) : (sdl))
