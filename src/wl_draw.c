...
void HitHorizDoor(void)
{
	...
	texture = ((xintercept - doorposition[doornum]) >> FIXED2TEXSHIFT) &
			  TEXTUREMASK;
...
void HitVertDoor(void)
{
	...
	doornum = tilehit & ~BIT_DOOR;
	texture = ((yintercept - doorposition[doornum]) >> FIXED2TEXSHIFT) &
			  TEXTUREMASK;
...
#ifdef FIXCALCROTATE
	viewangle = (int)(player->angle +
					  (centerx - ob->viewx) / (8 * viewwidth / 320.0));
#else
...
void DrawScaleds(void)
{
	...
	for (obj = player->next; obj; obj = obj->next) {
		...
		//
		if (*visspot || (*(visspot - 1) && !*(tilespot - 1)) ||
...
void WallRefresh(void)
{
	...
	for (pixx = 0; pixx < viewwidth; pixx++) {
		...
		while (1) {
			...
			//
			if ((ytilestep == -1 && yinttile <= ytile) ||
				(ytilestep == 1 && yinttile >= ytile))
				goto horizentry;
		vertentry:
			//
			// get the wall value from tilemap
			//
			if (tilemap[xtile][ytile] && (xtile - xtilestep) == xinttile &&
				(ytile - ytilestep) == yinttile) {
				//
				...
				if (tilehit & BIT_DOOR)
					passdoor =
							false; // don't let the trace continue if it's a door
			} else {
			...
			if (tilehit) {
				if (tilehit & BIT_DOOR) {
					...
					yinttemp =
							yintercept +
							(ystep >>
							 1); // add halfstep to current intercept position
					...
					if (pwalldir == di_west || pwalldir == di_east) {
						...
						if (pwalldir == di_east && xtile == pwallx &&
									yinttile == pwally ||
							pwalldir == di_west &&
									!(xtile == pwallx && yinttile == pwally)) {
							yinttemp =
									yintercept + ((ystep * pwallposnorm) >> 6);
							...
							yintercept = yinttemp;
							xintercept =
									(((fixed)xtile << TILESHIFT) + TILEGLOBAL) -
									(pwallposinv << 10);
							yinttile = yintercept >> TILESHIFT;
							tilehit = pwalltile;
							...
							yinttemp =
									yintercept + ((ystep * pwallposinv) >> 6);
		...
		while (1) {
			...
			//
			if ((xtilestep == -1 && xinttile <= xtile) ||
				(xtilestep == 1 && xinttile >= xtile))
				goto vertentry;
			...
			if (tilehit) {
				if (tilehit & BIT_DOOR) {
					...
					xinttemp =
							xintercept +
							(xstep >>
							 1); // add half step to current intercept position
					...
					if (pwalldir == di_north || pwalldir == di_south) {
						...
						if (pwalldir == di_south && xinttile == pwallx &&
									ytile == pwally ||
							pwalldir == di_north &&
									!(xinttile == pwallx && ytile == pwally)) {
							xinttemp =
									xintercept + ((xstep * pwallposnorm) >> 6);
							...
							xintercept = xinttemp;
							yintercept =
									(((fixed)ytile << TILESHIFT) + TILEGLOBAL) -
									(pwallposinv << 10);
							xinttile = xintercept >> TILESHIFT;
							tilehit = pwalltile;
							...
							xinttemp =
									xintercept + ((xstep * pwallposinv) >> 6);
