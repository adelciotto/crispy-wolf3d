...
void CAL_SetupGrFile(void)
{
	...
	if (!param_ignorenumchunks && headersize / 3 != (long)expectedsize)
		Quit("crispy-wolf3d was not compiled for these data files:\n"
	...
	CAL_HuffExpand(compseg, (byte *)pictable, NUMPICS * sizeof(*pictable),
				   grhuffman);
	free(compseg);
...
void CA_CacheAdlibSoundChunk(int chunk)
{
	...
	AdLibSound *sound =
			SafeMalloc(size + sizeof(*sound) - ORIG_ADLIBSOUND_SIZE);
	...
	read(audiohandle, sound->data,
		 size - ORIG_ADLIBSOUND_SIZE + 1); // + 1 because of byte data[1]
...
void CAL_DeplaneGrChunk(int chunk)
{
	...
	if (chunk == STARTTILE8) {
		...
		for (i = 0; i < NUMTILE8; i++)
			VL_DePlaneVGA(grsegs[chunk] + (i * (width * height)), width,
						  height);
	} else {
...
void CA_CacheMap(int mapnum)
{
	...
	if (mapheaderseg[mapnum]->width != MAPSIZE ||
		mapheaderseg[mapnum]->height != MAPSIZE)
