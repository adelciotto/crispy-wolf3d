...
void Write(int x, int y, const char *string)
{
	static const int alpha[] = {
		...
		L_NUM6PIC, L_NUM7PIC, L_NUM8PIC, L_NUM9PIC, L_COLONPIC, 0,
		0,		   0,		  0,		 0,			0,			L_APIC,
		L_BPIC,	   L_CPIC,	  L_DPIC,	 L_EPIC,	L_FPIC,		L_GPIC,
		L_HPIC,	   L_IPIC,	  L_JPIC,	 L_KPIC,	L_LPIC,		L_MPIC,
		L_NPIC,	   L_OPIC,	  L_PPIC,	 L_QPIC,	L_RPIC,		L_SPIC,
		L_TPIC,	   L_UPIC,	  L_VPIC,	 L_WPIC,	L_XPIC,		L_YPIC,
		L_ZPIC
	};
...
#define PERCENT100AMT 10000
	...
	times parTimes[] = {
		...
		//
		{ 1.5, "01:30" },  { 3.5, "03:30" },  { 2.75, "02:45" },
		{ 3.5, "03:30" },  { 0, "??:??" }, // Boss 1
		{ 4.5, "04:30" },  { 3.25, "03:15" }, { 2.75, "02:45" },
		{ 4.75, "04:45" }, { 0, "??:??" }, // Boss 2
		{ 6.5, "06:30" },  { 4.5, "04:30" },  { 2.75, "02:45" },
		{ 4.5, "04:30" },  { 6, "06:00" },	  { 0, "??:??" }, // Boss 3
		{ 6, "06:00" },	   { 0, "??:??" }, // Boss 4
		{ 0, "??:??" }, // Secret level 1
		{ 0, "??:??" }, // Secret level 2
#endif
...
#else
	if (gamestate.mapon != 4 && gamestate.mapon != 9 && gamestate.mapon != 15 &&
		gamestate.mapon < 17)
#endif
	{
#ifndef JAPAN
...
#ifdef SPANISH
		Write(30, 12,
			  parTimes[gamestate.episode * 10 + gamestate.mapon].timestr);
#else
		Write(26, 12,
			  parTimes[gamestate.episode * 10 + gamestate.mapon].timestr);
#endif
		...
		if (gamestate.TimeCount <
			...
			timeleft =
					(int32_t)((parTimes[gamestate.episode * 10 + gamestate.mapon]
									   .time *
							   4200) / 70 -
							  sec);
...
#define RATIOXX 37
	...
	//
		...
		bonus = (int32_t)timeleft * PAR_AMOUNT + (PERCENT100AMT * (kr >= 100)) +
...
boolean PreloadUpdate(unsigned current, unsigned total)
{
	...
	VWB_BarScaledCoord(WindowX + scaleFactor * 5,
					   WindowY + WindowH - scaleFactor * 3, w, scaleFactor * 2,
					   BLACK);
	w = ((int32_t)w * current) / total;
	if (w) {
		VWB_BarScaledCoord(WindowX + scaleFactor * 5,
						   WindowY + WindowH - scaleFactor * 3, w,
						   scaleFactor * 2,
						   0x37); // SECONDCOLOR);
		VWB_BarScaledCoord(WindowX + scaleFactor * 5,
						   WindowY + WindowH - scaleFactor * 3,
...
void PreloadGraphics(void)
{
	...
	VWB_BarScaledCoord(0, 0, screenWidth,
					   screenHeight - scaleFactor * (STATUSLINES - 1),
					   bordercol);
	VWB_DrawPicScaledCoord(((screenWidth - scaleFactor * 224) / 16) * 8,
						   (screenHeight - scaleFactor * (STATUSLINES + 48)) /
								   2,
						   GETPSYCHEDPIC);
...
void DrawHighScores(void)
{
	...
	for (i = 0, s = Scores; i < MaxScores; i++, s++) {
		...
		if (!i) {
			char temp =
					(((s->score >> 28) & 0xf) ^ ((s->score >> 24) & 0xf)) + 'A';
			char temp1 =
					(((s->score >> 20) & 0xf) ^ ((s->score >> 16) & 0xf)) + 'A';
			char temp2 =
					(((s->score >> 12) & 0xf) ^ ((s->score >> 8) & 0xf)) + 'A';
			char temp3 =
					(((s->score >> 4) & 0xf) ^ ((s->score >> 0) & 0xf)) + 'A';
...
char BackDoorStrs[5][16] = { "a spoon?", "bite me!", "joshua", "pelt",
							 "snoops" };
...
char bossstrs[4][24] = { "DEATH KNIGHT", "BARNACLE WILHELM",
						 "UBERMUTANTUBER MUTANT", "TRANS GROSSE" };

char WordStr[5][20] = { "New Game", "Sound...F4", "Control...F6",
						"Change View...F5", "Quit...F10" };
...
char MemberCorrect[5][24] = { "adrian carmack", "john carmackjohn romero",
							  "tom hall", "jay wilbur", "kevin cloud" };

char DosMessages[9][80] = { STR_NOPE1, STR_NOPE2, STR_NOPE3,
							STR_NOPE4, STR_NOPE5, STR_NOPE6,
							STR_NOPE7, STR_NOPE8, STR_NOPE9 };

char MiscTitle[4][20] = { "BLOOD TEST", "STRAIGHT-LACED", "QUITE SHAPELY",
						  "I AM WHAT I AMMO" };
	...
	while (attempt < 3) {
		...
		case debriefing: {
			...
			if (inputlen > 3) {
				...
				for (i = 0; i < bosslen; i++) {
					if (!strncasecmp(inputbuffer, bossstrs[whichboss] + i,
									 inputlen)) {
						match = 1;
						break;
					}
		...
		case staffquiz: {
			...
			if (inputlen > 2) {
				...
				for (i = 0; i < memberlen; i++) {
					if (!strncasecmp(inputbuffer, MemberCorrect[whichmem] + i,
									 inputlen)) {
						match = 1;
						break;
					}
