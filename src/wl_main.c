...
void ReadConfig(void)
{
	...
	if (configdir[0])
		snprintf(configpath, sizeof(configpath), "%s/%s", configdir,
				 configname);
	else
...
void WriteConfig(void)
{
	...
	if (file != -1) {
		...
		write(file, &dummyJoystickProgressive,
			  sizeof(dummyJoystickProgressive));
		int dummyJoystickPort = 0;
...
boolean SaveTheGame(FILE *file, int x, int y)
{
	...
	checksum = DoChecksum((byte *)&LevelRatios[0], sizeof(LRstruct) * LRpack,
						  checksum);
	...
	for (ob = ob->next; ob; ob = ob->next) {
		memcpy(&nullobj, ob, sizeof(nullobj));
		nullobj.state = (statetype *)((uintptr_t)nullobj.state -
									  (uintptr_t)&s_grdstand);
		fwrite(&nullobj, sizeof(nullobj), 1, file);
	}
	...
	checksum = DoChecksum((byte *)&laststatobjnum, sizeof(laststatobjnum),
						  checksum);
	...
	for (i = 0; i < MAXSTATS; i++) {
		...
		nullstat.visspot =
				(byte *)((uintptr_t)nullstat.visspot - (uintptr_t)spotvis);
...
boolean LoadTheGame(FILE *file, int x, int y)
{
	...
	player->state =
			(statetype *)((uintptr_t)player->state + (uintptr_t)&s_player);
	...
	if (oldchecksum != checksum) {
		...
		gamestate.lives = 1;
		gamestate.weapon = gamestate.chosenweapon = gamestate.bestweapon =
				wp_pistol;
		gamestate.ammo = 8;
	}
...
// These first sounds are in the upload version
#ifndef SPEAR
	HALTSND, 0, -1, DOGBARKSND, 1, -1, CLOSEDOORSND, 2, -1, OPENDOORSND, 3, -1,
	ATKMACHINEGUNSND, 4, 0, ATKPISTOLSND, 5, 0, ATKGATLINGSND, 6, 0,
	SCHUTZADSND, 7, -1, GUTENTAGSND, 8, -1, MUTTISND, 9, -1, BOSSFIRESND, 10, 1,
	SSFIRESND, 11, -1, DEATHSCREAM1SND, 12, -1, DEATHSCREAM2SND, 13, -1,
	DEATHSCREAM3SND, 13, -1, TAKEDAMAGESND, 14, -1, PUSHWALLSND, 15, -1,
	...
	TOT_HUNDSND, 23, -1, MEINGOTTSND, 24, -1, SCHABBSHASND, 25, -1, HITLERHASND,
	26, -1, SPIONSND, 27, -1, NEINSOVASSND, 28, -1, DOGATTACKSND, 29, -1,
	LEVELDONESND, 30, -1, MECHSTEPSND, 31, -1,
	...
	//
	HALTSND,		 0,	 -1, CLOSEDOORSND,	   2,  -1,
	OPENDOORSND,	 3,	 -1, ATKMACHINEGUNSND, 4,  0,
	ATKPISTOLSND,	 5,	 0,	 ATKGATLINGSND,	   6,  0,
	SCHUTZADSND,	 7,	 -1, BOSSFIRESND,	   8,  1,
	SSFIRESND,		 9,	 -1, DEATHSCREAM1SND,  10, -1,
	DEATHSCREAM2SND, 11, -1, TAKEDAMAGESND,	   12, -1,
	PUSHWALLSND,	 13, -1, AHHHGSND,		   15, -1,
	LEBENSND,		 16, -1, NAZIFIRESND,	   17, -1,
	SLURPIESND,		 18, -1, LEVELDONESND,	   22, -1,
...
#ifndef SPEARDEMO
	DOGBARKSND,		 1,	 -1, DOGDEATHSND,	   14, -1,
	SPIONSND,		 19, -1, NEINSOVASSND,	   20, -1,
...
CP_itemtype MusicMenu[] = { { 1, "Get Them!", 0 },
							{ 1, "Searching", 0 },
							{ 1, "P.O.W.", 0 },
							{ 1, "Suspense", 0 },
							{ 1, "War March", 0 },
...
CP_itemtype MusicMenu[] = {
	{ 1, "Funky Colonel Bill", 0 },		 { 1, "Death To The Nazis", 0 },
	{ 1, "Tiptoeing Around", 0 },		 { 1, "Is This THE END?", 0 },
	{ 1, "Evil Incarnate", 0 },			 { 1, "Jazzin' Them Nazis", 0 },
	{ 1, "Puttin' It To The Enemy", 0 }, { 1, "The SS Gonna Get You", 0 },
	{ 1, "Towering Above", 0 }
};
#endif
...
boolean SetViewSize(unsigned width, unsigned height)
{
	...
	if ((unsigned)viewheight == screenHeight)
		...
		viewscreeny =
				(screenHeight - scaleFactor * STATUSLINES - viewheight) / 2;
...
#ifdef NOTYET
#ifndef JAPAN
		memcpy((byte *)0xb8000, screen + 7,
			   24 * 160); // 24 for SPEAR/UPLOAD compatibility
#endif
		SetTextCursor(0, 23);
#endif
	}
	...
	for (i = 1; i < argc; i++) {
		...
		else IFARG("--joystick")
		{
			if (++i >= argc) {
				...
				param_joystickindex =
						atoi(argv[i]); // index is checked in InitGame
		}
