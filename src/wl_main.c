...
char str[80];
int dirangle[9] = {
    0,     ANGLES / 8, 2 * ANGLES / 8, 3 * ANGLES / 8, 4 * ANGLES / 8, 5 * ANGLES / 8, 6 * ANGLES / 8, 7 * ANGLES / 8,
    ANGLES};
...
// These first sounds are in the upload version
#ifndef SPEAR
    HALTSND, 0, -1, DOGBARKSND, 1, -1, CLOSEDOORSND, 2, -1, OPENDOORSND, 3, -1, ATKMACHINEGUNSND, 4, 0, ATKPISTOLSND, 5,
    0, ATKGATLINGSND, 6, 0, SCHUTZADSND, 7, -1, GUTENTAGSND, 8, -1, MUTTISND, 9, -1, BOSSFIRESND, 10, 1, SSFIRESND, 11,
    -1, DEATHSCREAM1SND, 12, -1, DEATHSCREAM2SND, 13, -1, DEATHSCREAM3SND, 13, -1, TAKEDAMAGESND, 14, -1, PUSHWALLSND,
    15, -1,
    ...
    TOT_HUNDSND, 23, -1, MEINGOTTSND, 24, -1, SCHABBSHASND, 25, -1, HITLERHASND, 26, -1, SPIONSND, 27, -1, NEINSOVASSND,
    28, -1, DOGATTACKSND, 29, -1, LEVELDONESND, 30, -1, MECHSTEPSND, 31, -1,
    ...
    //
    HALTSND,         0,  -1, CLOSEDOORSND,    2,  -1, OPENDOORSND,     3,  -1, ATKMACHINEGUNSND, 4,  0,
    ATKPISTOLSND,    5,  0,  ATKGATLINGSND,   6,  0,  SCHUTZADSND,     7,  -1, BOSSFIRESND,      8,  1,
    SSFIRESND,       9,  -1, DEATHSCREAM1SND, 10, -1, DEATHSCREAM2SND, 11, -1, TAKEDAMAGESND,    12, -1,
    PUSHWALLSND,     13, -1, AHHHGSND,        15, -1, LEBENSND,        16, -1, NAZIFIRESND,      17, -1,
    SLURPIESND,      18, -1, LEVELDONESND,    22, -1, DEATHSCREAM4SND, 23, -1, // AIIEEE
...
#ifndef SPEARDEMO
    DOGBARKSND,      1,  -1, DOGDEATHSND,     14, -1, SPIONSND,        19, -1, NEINSOVASSND,     20, -1,
    DOGATTACKSND,    21, -1, TRANSSIGHTSND,   29, -1, // Trans Sight
...
CP_itemtype MusicMenu[] = {
    {1, "Funky Colonel Bill", 0},      {1, "Death To The Nazis", 0},   {1, "Tiptoeing Around", 0},
...
void CheckParameters(int argc, char *argv[])
{
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        else IFARG("--baby") param_difficulty = 0;
        else IFARG("--easy") param_difficulty = 1;
        else IFARG("--normal") param_difficulty = 2;
        ...
        else IFARG("--tedlevel")
        {
            if (++i >= argc)
            {
                CrispyLogError("The tedlevel option is missing the level argument!\n");
                hasError = true;
            }
            else
        ...
        {
            PARSE_BOOL_ARG("fullscreen", CrispyConfigFullscreen);
        }
        ...
        else IFARG("--window_scale")
        {
            if (++i >= argc)
            {
                CrispyLogError("The window_scale option is missing the scale argument!\n");
                hasError = true;
            }
            else
            {
                ...
                if (val > 0 && val < 5)
                {
                    CrispyConfigWindowScale = val;
                    CrispyLogInfo("Config 'window_scale' overridden to %u", CrispyConfigWindowScale);
                }
                else
                {
                    CrispyLogError("The window_scale option argument must be an integer between 1 and 4!\n");
                    hasError = true;
                }
            }
        }
        ...
        {
            PARSE_BOOL_ARG("high_res", CrispyConfigHighRes);
        }
        ...
        else IFARG("--joystick")
        {
            if (++i >= argc)
            {
                CrispyLogError("The joystick option is missing the index argument!\n");
                hasError = true;
            }
            else
        ...
        else IFARG("--joystickhat")
        {
            if (++i >= argc)
            {
                CrispyLogError("The joystickhat option is missing the index argument!\n");
                hasError = true;
            }
            else
        ...
        else IFARG("--samplerate")
        {
            if (++i >= argc)
            {
                CrispyLogError("The samplerate option is missing the rate argument!\n");
                hasError = true;
            }
            else
        ...
        else IFARG("--audiobuffer")
        {
            if (++i >= argc)
            {
                CrispyLogError("The audiobuffer option is missing the size argument!\n");
                hasError = true;
            }
            else
        ...
        else IFARG("--mission")
        {
            if (++i >= argc)
            {
                CrispyLogError("The mission option is missing the mission argument!\n");
                hasError = true;
            }
            else
            {
                ...
                {
                    CrispyLogError("The mission option must be between 0 and 3!\n");
                    hasError = true;
                }
            }
        }
        else IFARG("--configdir")
        {
            if (++i >= argc)
            {
                CrispyLogError("The configdir option is missing the dir argument!\n");
                hasError = true;
            }
            else
            {
        ...
        else IFARG("--goodtimes") param_goodtimes = true;
        else IFARG("--ignorenumchunks") param_ignorenumchunks = true;
...
void FindOrCreateConfig()
{
    struct stat statbuf;

    // On Linux like systems, the configdir defaults to $HOME/.crispy-wolf3d
#if !defined(_WIN32)
    if (configdir[0] == 0)
    {
