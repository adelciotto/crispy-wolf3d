...
void SpawnStatic(int tilex, int tiley, int type)
{
	...
	case block:
		actorat[tilex][tiley] =
				(objtype *)BIT_WALL; // consider it a blocking tile
	case none:
		laststatobj->flags = 0;
		break;
...
void SpawnDoor(int tilex, int tiley, boolean vertical, int lock)
{
	...
	actorat[tilex][tiley] = (objtype *)(uintptr_t)(
			doornum | BIT_DOOR); // consider it a solid wall
...
void DoorClosing(int door)
{
	...
	if (((int)(uintptr_t)actorat[tilex][tiley] != (door | BIT_DOOR)) ||
		(player->tilex == tilex &&
		 player->tiley == tiley)) { // something got inside the door
		OpenDoor(door);
		return;
	};
...
void PushWall(int checkx, int checky, int dir)
{
	...
	actorat[checkx + dx][checky + dy] =
			(objtype *)(uintptr_t)(tilemap[checkx + dx][checky + dy] = oldtile);
	...
	MAPSPOT(pwallx, pwally, 0) = MAPSPOT(
			player->tilex, player->tiley,
			0); // set correct floorcode (BrotherTank's fix) TODO: use a better method...
...
void MovePWalls(void)
{
	...
	if (pwallstate / 128 != oldblock) {
		...
		MAPSPOT(pwallx, pwally, 0) =
				player->areanumber +
				AREATILE; // TODO: this is unnecessary, and makes a mess of mapsegs
		...
		if (pwallstate >= 256) // only move two tiles fix
		{
			...
			if (actorat[pwallx + dx][pwally + dy] ||
				xl <= pwallx + dx && pwallx + dx <= xh && yl <= pwally + dy &&
						pwally + dy <= yh) {
				pwallstate = 0;
				tilemap[pwallx][pwally] = oldtile;
				return;
			}
			actorat[pwallx + dx][pwally + dy] = (objtype *)(uintptr_t)(
					tilemap[pwallx + dx][pwally + dy] = oldtile);
			tilemap[pwallx + dx][pwally + dy] = BIT_WALL;
		}
	}
